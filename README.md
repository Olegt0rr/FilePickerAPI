...
### `GET /files`
Получить список всех файлов в настроенной директории, отфильтрованных по размеру.

Файлы разделяются на две категории:
- **availableFiles** - файлы размером меньше 10 МБ (доступные для импорта)
- **notAvailableFiles** - файлы размером 10 МБ и больше (не доступные для импорта)
